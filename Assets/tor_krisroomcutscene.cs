		...
		if (torieltext.currentDialogueBox == null)
		{
			...
			else
			{
				...
				else
				{
					...
					if (seen)
					{
						torieltext.transform.position = Vector3.MoveTowards(torieltext.transform.position, new Vector3(torieltext.transform.position.x, -500, torieltext.transform.position.z), Time.deltaTime * 3f);
					}
